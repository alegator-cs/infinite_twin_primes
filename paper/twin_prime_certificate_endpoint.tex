...
\title{Twin Prime Conditional Endpoint by Finite Contradiction}
...
\end{center}
\noindent
We formalize the Lean endpoint for a finite-contradiction approach to twin
prime infinitude.  The checked core proves that supposing the finiteness of
twins forces a cofinal tail of midpoint-exceptional primes, and that any
cofinal tail contradicts a moving-window MP/PM event-pressure certificate.  The
remaining mathematical content is the pressure certificate itself: fresh
eligible exceptional starts must create enough distinct composite-prime and
prime-composite middle-pair events in the finite moving window.  The generated
routed-chain shards provide audit data for this pressure picture, but the
generic fresh-start and descent theorems alone do not yet prove fresh event
pressure.
...
checks that a suitable event-pressure theorem for that tail rules out the tail,
and then gives arbitrarily large twin primes.  The present repository should be
read as a precise conditional endpoint plus finite audit data, not as a closed
unconditional proof.
...
generated C++ certificate emits the arithmetic route chains; the semantic
realization step is represented in Lean by the explicit certificate type
\[
  \Lean{RoutedChainRealizationCertificate}.
\]
Given such a certificate, Lean derives
\[
  \Lean{GeneratedTailInductionCertificate.no\_cofinalExceptionTail\_of\_routedChainRealization}
\]
...
route-realization certificate is ordinary Lean proof.  The routed-chain endpoint is
\[
  \Lean{GeneratedTailInductionCertificate.arbitrarily\_large\_twins\_of\_routedChainRealization}.
\]
Given a value of \Lean{RoutedChainRealizationCertificate}, its statement expands to:
\[
...
\[
  \Lean{no\_cofinalExceptionTail\_of\_routedMPPMChainCertificate}.
\]
...
\begin{filelist}
  ...
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    ...
    \item \Lean{RoutedChainRealizationCertificate}: the explicit semantic realization certificate consumed by the final endpoint.
    \item \Lean{no\_cofinalExceptionTail\_of\_routedMPPMChainCertificate}: the finite-set contradiction from that certificate.
  \end{itemize}

  \FileEntry{TwinPrimeCertificate/GeneratedTailInductionCertificate.lean}
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    \item \Lean{fromRoutedChainRealization}: converts the routed-chain realization certificate into the tail-induction interface.
    \item \Lean{no\_cofinalExceptionTail\_of\_routedChainRealization}: derives no cofinal exceptional tail.
    \item \Lean{arbitrarily\_large\_twins\_of\_routedChainRealization}: the routed-chain twin-prime endpoint.
  \end{itemize}
...
The main generated finite certificate is expressed in Lean as the
route-realization bridge
\begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
  \item \Lean{RoutedChainRealizationCertificate}
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
  \item \Lean{GeneratedTailInductionCertificate.no\_cofinalExceptionTail\_of\_routedChainRealization}
\end{itemize}
whose mathematical shape is
\[
\Not\ \exists B,\ 
...
The C++ program \Code{generate\_routed\_mppm\_chain\_certificate.cpp} generates
...
\begin{filelist}
  ...
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    ...
    \item \Lean{RoutedChainRealizationCertificate}: the semantic realization certificate.
    \item \Lean{no\_cofinalExceptionTail\_of\_routedMPPMChainCertificate}: packages the cardinal contradiction as a no-tail theorem.
  \end{itemize}

  \FileEntry{TwinPrimeCertificate/GeneratedTailInductionCertificate.lean}
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    \item \Lean{fromRoutedChainRealization}: converts the routed-chain realization certificate into the tail-induction interface.
    \item \Lean{no\_cofinalExceptionTail\_of\_routedChainRealization}: the derived no-tail theorem.
    \item \Lean{arbitrarily\_large\_twins\_of\_routedChainRealization}: the routed-chain twin-prime endpoint.
  \end{itemize}
\end{filelist}
...
is simple: if one arbitrarily late parent exists after every bound, then ask
once after \(N\) and once after the first parent.  This gives two distinct
eligible parents, which is exactly the multiplicity-two hypothesis used by
the shifted-tail recovery theorem.
...
\begin{theorem}[Routed-chain certificate implies arbitrarily large twin primes]
Given a value of \(\Lean{RoutedChainRealizationCertificate}\),
\[
  \forall N,\ \exists p,\ N\le p\wedge\Prime(p)\wedge\Prime(p+2).
\]
\end{theorem}
...
\begin{proof}
Let \(\mathcal C\) be the routed-chain realization certificate.
...
\begin{filelist}
  ...
  \FileEntry{TwinPrimeCertificate/GeneratedTailInductionCertificate.lean}
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    \item \Lean{no\_cofinalExceptionTail\_of\_routedChainRealization}: supplies the no-tail theorem used by the main endpoint.
  \end{itemize}
...
\Lean{RoutedChainRealizationCertificate}.  Lean reports only the standard axioms
...
\begin{filelist}
  \FileEntry{TwinPrimeCertificate/GeneratedTailInductionCertificate.lean}
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    \item \Lean{fromRoutedChainRealization}: builds the midpoint tail-induction certificate.
    \item \Lean{no\_cofinalExceptionTail\_of\_routedChainRealization}: the no-tail endpoint from the routed-chain realization certificate.
    \item \Lean{arbitrarily\_large\_twins\_of\_routedChainRealization}: the theorem inspected by \Code{\#print axioms}.
  \end{itemize}

  \FileEntry{TwinPrimeCertificate/RoutedMPPMChainBridge.lean}
  \begin{itemize}[leftmargin=1.5em,itemsep=0.15em]
    \item \Lean{RoutedChainRealizationCertificate}: the explicit certificate parameter of the audited theorem.
    \item \Lean{no\_cofinalExceptionTail\_of\_routedMPPMChainCertificate}: the finite cardinality contradiction consumed by the endpoint.
  \end{itemize}
\end{filelist}
...
\begin{itemize}
  ...
  \item \Code{TwinPrimeCertificate/FiniteSinkAvoidance.lean}: contains the
  finite-sink, moving-window event-pressure, pair-pressure, terminal-slot, and
  ancestor-bridge endpoint surfaces.
  ...
  contradiction, relative to \Lean{RoutedChainRealizationCertificate}.
  ...
  \Lean{RoutedChainRealizationCertificate} to \Lean{Final.lean} and exposes the
  routed-chain arbitrarily-large-twins theorem.

  \item \Code{TwinPrimeCertificate/UniqueDescentEndpoint.lean}: exposes the
  unique moving-window descent, injective descent, and moving-window
  event-pressure theorem surfaces.
\end{itemize}
...
\begin{itemize}
  ...
  \item \Code{research/pressure\_overflow\_program.md}: running research log for
  the pressure, recovery, and DFI--Toth root-supply route.
...
\subsection{Finite-sink and tail-induction surfaces}

\begin{filelist}
  \FileEntry{TwinPrimeCertificate/FiniteSinkAvoidance.lean}
  \begin{objectlist}
    \Obj{finite\_ancestor\_set\_misses\_arbitrarily\_large\_modFiveOnePrime}
    \Obj{finite\_ancestor\_set\_not\_cofinal\_for\_modFiveOnePrimes}
    \Obj{exists\_modFiveOnePrime\_batch\_after}
    \Obj{FiniteSinkAvoidanceCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_finiteSinkAvoidance}
    \Obj{arbitrarily\_large\_twins\_of\_finiteSinkAvoidance}
    \Obj{ResidueFiniteSinkAvoidanceCertificate}
    \Obj{ResidueFiniteSinkAvoidanceCertificate.toFiniteSinkAvoidance}
    \Obj{no\_cofinalExceptionTail\_of\_residueFiniteSinkAvoidance}
    \Obj{arbitrarily\_large\_twins\_of\_residueFiniteSinkAvoidance}
    \Obj{TerminalSlotSinkCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_terminalSlotSink}
    \Obj{arbitrarily\_large\_twins\_of\_terminalSlotSink}
    \Obj{slotAncestorUnion}
    \Obj{TerminalSlotAncestorBridgeCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_terminalSlotAncestorBridge}
    \Obj{arbitrarily\_large\_twins\_of\_terminalSlotAncestorBridge}
    \Obj{movingSlotAncestorUnion}
    \Obj{MovingTerminalSlotAncestorBridgeCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_movingTerminalSlotAncestorBridge}
    \Obj{arbitrarily\_large\_twins\_of\_movingTerminalSlotAncestorBridge}
    \Obj{no\_cofinalExceptionTail\_of\_uniqueMovingWindowDescent}
    \Obj{arbitrarily\_large\_twins\_of\_uniqueMovingWindowDescent}
    \Obj{no\_cofinalExceptionTail\_of\_injectiveMovingWindowDescent}
    \Obj{arbitrarily\_large\_twins\_of\_injectiveMovingWindowDescent}
    \Obj{MovingWindowEventPressureCertificate}
    \Obj{PairEventPressureCertificate}
    \Obj{PairEventPressureCertificate.toMovingWindowEventPressure}
    \Obj{no\_cofinalExceptionTail\_of\_movingWindowEventPressure}
    \Obj{arbitrarily\_large\_twins\_of\_movingWindowEventPressure}
    \Obj{no\_cofinalExceptionTail\_of\_pairEventPressure}
    \Obj{arbitrarily\_large\_twins\_of\_pairEventPressure}
  \end{objectlist}
  ...
  \FileEntry{TwinPrimeCertificate/UniqueDescentEndpoint.lean}
  \begin{objectlist}
    \Obj{fromUniqueMovingWindowDescent}
    \Obj{no\_cofinalExceptionTail\_of\_uniqueMovingWindowDescent}
    \Obj{arbitrarily\_large\_twins\_of\_uniqueMovingWindowDescent}
    \Obj{fromInjectiveMovingWindowDescent}
    \Obj{no\_cofinalExceptionTail\_of\_injectiveMovingWindowDescent}
    \Obj{arbitrarily\_large\_twins\_of\_injectiveMovingWindowDescent}
    \Obj{fromMovingWindowEventPressure}
    \Obj{no\_cofinalExceptionTail\_of\_movingWindowEventPressure}
    \Obj{arbitrarily\_large\_twins\_of\_movingWindowEventPressure}
  \end{objectlist}
\end{filelist}
...
\begin{filelist}
  \FileEntry{TwinPrimeCertificate/GeneratedTailInductionCertificate.lean}
  \begin{objectlist}
    \Obj{GeneratedFinitePrefixRecoveryCertificate}
    \Obj{GeneratedSuccessorRecoveryCertificate}
    \Obj{fromGeneratedFinitePrefixRecovery}
    \Obj{fromGeneratedSuccessorRecovery}
    \Obj{no\_cofinalExceptionTail\_of\_generatedFinitePrefixRecovery}
    \Obj{arbitrarily\_large\_twins\_of\_generatedFinitePrefixRecovery}
    ...
    \Obj{fromRoutedChainRealization}
    \Obj{no\_cofinalExceptionTail\_of\_routedChainRealization}
    \Obj{arbitrarily\_large\_twins\_of\_routedChainRealization}
    \Obj{fromModFiveSeedOverflow}
    \Obj{no\_cofinalExceptionTail\_of\_modFiveSeedOverflow}
    \Obj{arbitrarily\_large\_twins\_of\_modFiveSeedOverflow}
    \Obj{fromModFiveGoodSeedOverflow}
    \Obj{no\_cofinalExceptionTail\_of\_modFiveGoodSeedOverflow}
    \Obj{arbitrarily\_large\_twins\_of\_modFiveGoodSeedOverflow}
    \Obj{fromTerminalSlotSink}
    \Obj{no\_cofinalExceptionTail\_of\_terminalSlotSink}
    \Obj{arbitrarily\_large\_twins\_of\_terminalSlotSink}
    \Obj{fromTerminalSlotAncestorBridge}
    \Obj{no\_cofinalExceptionTail\_of\_terminalSlotAncestorBridge}
    \Obj{arbitrarily\_large\_twins\_of\_terminalSlotAncestorBridge}
    \Obj{fromMovingTerminalSlotAncestorBridge}
    \Obj{no\_cofinalExceptionTail\_of\_movingTerminalSlotAncestorBridge}
    \Obj{arbitrarily\_large\_twins\_of\_movingTerminalSlotAncestorBridge}
    \Obj{fromUniqueMovingWindowDescent}
    \Obj{no\_cofinalExceptionTail\_of\_uniqueMovingWindowDescent}
  ...
  \begin{objectlist}
    ...
    \Obj{RoutedChainRealizationCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_routedMPPMChainCertificate}
    \Obj{ModFiveSeedOverflowCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_modFiveSeedOverflowCertificate}
    \Obj{ModFiveGoodSeedOverflowCertificate}
    \Obj{no\_cofinalExceptionTail\_of\_modFiveGoodSeedOverflowCertificate}
  \end{objectlist}
\end{filelist}
